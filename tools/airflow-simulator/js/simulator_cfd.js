...
function updateParticlesCFD() {
    ...
    particles.forEach(particle => {
        ...
        const verticalDistance = Math.abs(particle.position.y - fanCenter.y);
        ...
        // Determine if particle is near any boundary (SMACNA boundary layer standard)
        const nearBoundary = (
        ...
        // Reset velocity for fresh calculation
        let newVelocity = new THREE.Vector3(0, 0, 0);
        
        // 1. Fan influence (primary airflow vector) - based on AMCA standards
        if (horizontalDistance < fanInfluenceRadius) {
            // Calculate fan influence factor (decreases with distance)
            const fanInfluenceFactor = Math.max(0, 1 - (horizontalDistance / fanInfluenceRadius));
            
            if (fanConfig.direction === 'down') {
                // Downward flow from fan
                if (particle.position.y < fanCenter.y) {
                    // Below fan - strong downward and outward flow
                    // Calculate direction vector from fan center (horizontally)
                    ...
                    // Downward velocity component (strongest directly under fan)
                    ...
                    // Outward velocity component (increases as air approaches floor)
