// Industrial Fan Airflow Simulator - Fixed and Optimized Version
// Complete implementation with proper loading, visualization, and fan model
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize the simulator with a slight delay to ensure all resources are loaded
    setTimeout(function() {
        try {
            initSimulator();
...
function setupEventListeners() {
    ...
    // Input fields
    document.getElementById('room-length').addEventListener('change', function() {
        roomDimensions.length = parseFloat(this.value);
    });
    
    document.getElementById('room-width').addEventListener('change', function() {
        roomDimensions.width = parseFloat(this.value);
    });
    
    document.getElementById('room-height').addEventListener('change', function() {
        roomDimensions.height = parseFloat(this.value);
    });
    
    document.getElementById('fan-diameter').addEventListener('change', function() {
        fanConfig.diameter = parseFloat(this.value);
    });
    
    document.getElementById('fan-cfm').addEventListener('change', function() {
        fanConfig.cfm = parseFloat(this.value);
    });
    
    document.getElementById('fan-rpm').addEventListener('change', function() {
        fanConfig.rpm = parseFloat(this.value);
    });
    
    document.getElementById('fan-direction').addEventListener('change', function() {
        fanConfig.direction = this.value;
    });
    
    document.getElementById('fan-x').addEventListener('change', function() {
        fanConfig.position.x = parseFloat(this.value);
    });
    
    document.getElementById('fan-y').addEventListener('change', function() {
        fanConfig.position.y = parseFloat(this.value);
    });
    
    document.getElementById('fan-height').addEventListener('change', function() {
        fanConfig.position.z = parseFloat(this.value);
    });
    
    document.getElementById('particle-density').addEventListener('change', function() {
        particleDensity = this.value;
    });
    ...
    // Compare fans button
    document.getElementById('compare-fans').addEventListener('click', showFanComparison);
}
...
function createParticles() {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        if (distributionStrategy < 0.3) {
            ...
            // Height distribution along walls
            y = Math.random() * roomDimensions.height;
        } else {
            // 40% of particles distributed throughout room
            x = Math.random() * roomDimensions.length;
            y = Math.random() * roomDimensions.height;
            z = Math.random() * roomDimensions.width;
        }
        ...
        // Add CFD properties to particle
...
function animate() {
    ...
    // Rotate fan blades if playing
    if (isPlaying) {
        // Rotate blades based on fan RPM
        const rotationSpeed = (fanConfig.rpm / 60) * 0.1 * Math.PI;
        blades.forEach(blade => {
            blade.rotation.z += rotationSpeed;
        });
        
        // Update particle positions using CFD principles
        updateParticlesCFD();
    }
...
function onWindowResize() {
    camera.aspect = document.querySelector('.simulator-display').offsetWidth / 
                    document.querySelector('.simulator-display').offsetHeight;
...
function updateSimulation() {
    ...
    // Recreate room, fan, and particles
    createRoom();
    createFan();
    createParticles();
...
function updateResults() {
    ...
    // Calculate energy usage (kW)
    // Based on typical HVLS fan efficiency
    ...
    // Based on typical HVLS fan cooling effect
    const coolingEffect = 4 + (fanConfig.diameter / 2) * (fanConfig.rpm / 50);
...
function exportResults() {
    ...
    // Gather all simulation parameters and results
    const data = {
        // Room configuration
        roomLength: roomDimensions.length,
        roomWidth: roomDimensions.width,
        roomHeight: roomDimensions.height,
        roomVolume: roomDimensions.length * roomDimensions.width * roomDimensions.height,
        
        // Fan configuration
        fanModel: fanConfig.model,
        fanDiameter: fanConfig.diameter,
        fanCFM: fanConfig.cfm,
        fanRPM: fanConfig.rpm,
        fanDirection: fanConfig.direction,
        fanPositionX: fanConfig.position.x,
        fanPositionY: fanConfig.position.y,
        fanHeight: fanConfig.position.z,
        
        // Results
        coverageArea: Math.PI * Math.pow(fanConfig.diameter * 1.5, 2),
        airChanges: (fanConfig.cfm * 60) / (roomDimensions.length * roomDimensions.width * roomDimensions.height),
        energyUsage: (fanConfig.diameter / 10) * (fanConfig.rpm / 50) * 0.75,
        coolingEffect: 4 + (fanConfig.diameter / 2) * (fanConfig.rpm / 50)
    };
    ...
    // Room configuration
    csvContent += "Room Length," + data.roomLength + ",ft\n";
    csvContent += "Room Width," + data.roomWidth + ",ft\n";
    csvContent += "Room Height," + data.roomHeight + ",ft\n";
    csvContent += "Room Volume," + data.roomVolume.toFixed(1) + ",cu ft\n";
    
    // Fan configuration
    csvContent += "Fan Model," + data.fanModel + ",\n";
    csvContent += "Fan Diameter," + data.fanDiameter + ",ft\n";
    csvContent += "Fan CFM," + data.fanCFM + ",CFM\n";
    csvContent += "Fan RPM," + data.fanRPM + ",RPM\n";
    csvContent += "Fan Direction," + data.fanDirection + ",\n";
    csvContent += "Fan Position X," + data.fanPositionX + ",ft\n";
    csvContent += "Fan Position Y," + data.fanPositionY + ",ft\n";
    csvContent += "Fan Height," + data.fanHeight + ",ft\n";
    
    // Results
    csvContent += "Coverage Area," + Math.round(data.coverageArea) + ",sq ft\n";
    csvContent += "Air Changes," + data.airChanges.toFixed(1) + ",ACH\n";
    csvContent += "Energy Usage," + data.energyUsage.toFixed(2) + ",kW\n";
    csvContent += "Cooling Effect," + data.coolingEffect.toFixed(1) + ",°F\n";
...
// Show fan comparison interface
function showFanComparison() {
    console.log("Showing fan comparison interface...");
            ...
            <div class="comparison-container">
                <div class="comparison-column">
                    ...
                    <div class="comparison-data">
                        <p><strong>Model:</strong> ${fanConfig.model}</p>
                        <p><strong>Diameter:</strong> ${fanConfig.diameter} ft</p>
                        <p><strong>CFM:</strong> ${fanConfig.cfm}</p>
                        <p><strong>RPM:</strong> ${fanConfig.rpm}</p>
                    </div>
                    <div class="comparison-results">
                        <p><strong>Coverage:</strong> ${Math.round(Math.PI * Math.pow(fanConfig.diameter * 1.5, 2))} sq ft</p>
                        <p><strong>Air Changes:</strong> ${((fanConfig.cfm * 60) / (roomDimensions.length * roomDimensions.width * roomDimensions.height)).toFixed(1)} ACH</p>
                        <p><strong>Energy Usage:</strong> ${((fanConfig.diameter / 10) * (fanConfig.rpm / 50) * 0.75).toFixed(2)} kW</p>
                        <p><strong>Cooling Effect:</strong> ${(4 + (fanConfig.diameter / 2) * (fanConfig.rpm / 50)).toFixed(1)}°F</p>
                    </div>
                </div>
                ...
                <div class="comparison-column">
                    ...
                    <div class="comparison-results" id="comparison-results">
                        <p><strong>Coverage:</strong> ${Math.round(Math.PI * Math.pow(16 * 1.5, 2))} sq ft</p>
                        <p><strong>Air Changes:</strong> ${((350000 * 60) / (roomDimensions.length * roomDimensions.width * roomDimensions.height)).toFixed(1)} ACH</p>
                        <p><strong>Energy Usage:</strong> ${((16 / 10) * (45 / 50) * 0.75).toFixed(2)} kW</p>
                        <p><strong>Cooling Effect:</strong> ${(4 + (16 / 2) * (45 / 50)).toFixed(1)}°F</p>
                    </div>
                </div>
            </div>
...
function updateComparison() {
    console.log("Updating fan comparison...");
    
    // Get comparison fan parameters
    ...
    const compareAirChanges = (compareCFM * 60) / (roomDimensions.length * roomDimensions.width * roomDimensions.height);
    const compareEnergyUsage = (compareDiameter / 10) * (compareRPM / 50) * 0.75;
    const compareCoolingEffect = 4 + (compareDiameter / 2) * (compareRPM / 50);
    
    // Update comparison results display
    const resultsDiv = document.getElementById('comparison-results');
    resultsDiv.innerHTML = `
        <p><strong>Coverage:</strong> ${Math.round(compareCoverage)} sq ft</p>
        <p><strong>Air Changes:</strong> ${compareAirChanges.toFixed(1)} ACH</p>
        <p><strong>Energy Usage:</strong> ${compareEnergyUsage.toFixed(2)} kW</p>
        <p><strong>Cooling Effect:</strong> ${compareCoolingEffect.toFixed(1)}°F</p>
    `;
    
    // Update chart
    updateComparisonChart(compareCoverage, compareAirChanges, compareEnergyUsage, compareCoolingEffect);
}

// Create comparison chart
function createComparisonChart() {
    // Calculate current fan metrics
    const currentCoverage = Math.PI * Math.pow(fanConfig.diameter * 1.5, 2);
    const currentAirChanges = (fanConfig.cfm * 60) / (roomDimensions.length * roomDimensions.width * roomDimensions.height);
    const currentEnergyUsage = (fanConfig.diameter / 10) * (fanConfig.rpm / 50) * 0.75;
    const currentCoolingEffect = 4 + (fanConfig.diameter / 2) * (fanConfig.rpm / 50);
    
    // Calculate comparison fan metrics (initial values)
    const compareDiameter = 16;
    const compareCFM = 350000;
    const compareRPM = 45;
    ...
    window.comparisonChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: ['Coverage (sq ft)', 'Air Changes (ACH)', 'Energy Usage (kW)', 'Cooling Effect (°F)'],
            datasets: [
                {
                    label: 'Current Fan',
                    data: [
                        Math.round(currentCoverage),
                        parseFloat(currentAirChanges.toFixed(1)),
                        parseFloat(currentEnergyUsage.toFixed(2)),
                        parseFloat(currentCoolingEffect.toFixed(1))
                    ],
                ...
                {
                    label: 'Comparison Fan',
                    data: [
                        Math.round(compareCoverage),
                        parseFloat(compareAirChanges.toFixed(1)),
                        parseFloat(compareEnergyUsage.toFixed(2)),
                        parseFloat(compareCoolingEffect.toFixed(1))
                    ],
...
function updateComparisonChart(compareCoverage, compareAirChanges, compareEnergyUsage, compareCoolingEffect) {
    // Calculate current fan metrics
    const currentCoverage = Math.PI * Math.pow(fanConfig.diameter * 1.5, 2);
    const currentAirChanges = (fanConfig.cfm * 60) / (roomDimensions.length * roomDimensions.width * roomDimensions.height);
    const currentEnergyUsage = (fanConfig.diameter / 10) * (fanConfig.rpm / 50) * 0.75;
    ...
    window.comparisonChart.data.datasets[1].data = [
        Math.round(compareCoverage),
        parseFloat(compareAirChanges.toFixed(1)),
        parseFloat(compareEnergyUsage.toFixed(2)),
        parseFloat(compareCoolingEffect.toFixed(1))
